export interface RecipeParsingSettings {
  ...
  urlExtractionPrompt: string;
  shoppingListPrompt: string;
  deleteImagesAfterProcessing: boolean;
}
...
export const DEFAULT_SETTINGS: RecipeParsingSettings = {
  ...
  urlExtractionPrompt:
    "Extract the recipe from the provided webpage content. Return markdown content (do not surround it with backticks, only return the raw text), in the format:\n    <newline>\n# Ingredients\n## Component A(if appropriate)\n  Serves x(if given)\n  - ingredient\n  - ingredient\n  - ingredient\n\n# Recipe transcription\n  < copy of the recipe instructions > ",
  shoppingListPrompt:
    "You create a shopping list from recipe notes. Return markdown content (do not surround it with backticks, only return the raw text) matching the provided shopping list section exactly, and replace the empty checklist items with consolidated ingredients from both the attached recipies, and any items noted in the main meal plan document. Some recipes will have an indication that they will be cooked at double quantity (e.g. 'x2' or 'twice'), and some will have an indication that the recipe will need to be halved (e.g. '/2' or 'halved'), and the ingredients list will need to reflect that. Items must be grouped under the existing headings. If an item fits multiple headings, or is a poor fit for all of them, pick the best fit. Keep the '# Need to buy' header and the category headers unchanged. Keep checklist markers and put quantities in italics, '- [ ] <ingredient>: *<quantity>*'.",
  deleteImagesAfterProcessing: false
};
